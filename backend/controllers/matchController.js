...
export const updateMatchStatus = async (req, res) => {
  try {
    ...
    await Notification.create({
      ...
      // Auto-create chat and placeholder message when trade is accepted
      if (status === 'accepted') {
        // Import Conversation and Message models if not already
        ...
        const message = await Message.create({
          ...
          type: 'text',
          content: 'hey start the conversation',
          attachments: []
        });

        // Update conversation's lastMessage and unreadCount
        conversation.lastMessage = message._id;
        const unreadCount = conversation.unreadCount.get(otherUserId.toString()) || 0;
        conversation.unreadCount.set(otherUserId.toString(), unreadCount + 1);
